...
### Se aparecer erro de `pydantic-core` no Windows

Esse erro normalmente acontece quando:
- Python fora da faixa suportada para o build local (ex.: 3.13/3.14 sem wheel disponível);
- ambiente com Python 32-bit;
- `pip` antigo tentando compilar do fonte.

Correção aplicada neste projeto:
- script exige Python **3.11 ou 3.12 (x64)**;
- atualização de `pip/setuptools/wheel`;
- instalação com `--only-binary=:all:` para evitar build nativo.

### Comandos corretos (separados)

> Execute cada comando em uma linha separada; não concatene os comandos.
...
### Erro: `Use Python 3.11 ou 3.12 x64. Versao atual: 3.14.2`

Isso significa que o Python instalado não é compatível com este build atualmente.
...
4. Execute novamente na raiz:

```powershell
.\desktop\scripts\build-backend.ps1
```

### Erro: `Could not open requirements file: requirements.txt`

Esse erro ocorria quando o script seguia mesmo após falha anterior e/ou fora da pasta correta.
